...
public:
    ...
    Mesh() {
        ...
        ka = kd = 0.75f;
        boundingCenter = vec4(0.f, 0.f, 0.f, 1.f);
        boundingRadius = 0.f;
    }
    ...
    void updateBounds()
    {
        ...
        // 1) Find min/max X/Y/Z among all vertices
        float minx = FLT_MAX, miny = FLT_MAX, minz = FLT_MAX;
        float maxx = -FLT_MAX, maxy = -FLT_MAX, maxz = -FLT_MAX;

        for (auto& v : vertices) {
            if (v.p[0] < minx) minx = v.p[0];
            if (v.p[1] < miny) miny = v.p[1];
            if (v.p[2] < minz) minz = v.p[2];

            if (v.p[0] > maxx) maxx = v.p[0];
            if (v.p[1] > maxy) maxy = v.p[1];
            if (v.p[2] > maxz) maxz = v.p[2];
        }

        // 2) Midpoint => boundingCenter (still in object space)
        float cx = 0.5f * (minx + maxx);
        float cy = 0.5f * (miny + maxy);
        float cz = 0.5f * (minz + maxz);

        boundingCenter = vec4(cx, cy, cz, 1.f);

        // 3) Radius => max distance from center
        ...
        boundingRadius = std::sqrt(maxDistSqr);
    }
